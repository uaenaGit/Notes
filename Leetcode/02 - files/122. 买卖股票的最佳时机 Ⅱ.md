...
> to optimize 3rd：
![image.png](https://cdn.jsdelivr.net/gh/uaenaGit/image-host@main/images20260228120735284.png)
...
public:
    int maxProfit(vector<int>& prices) {
        int n = prices.size();
        vector<vector<int>> f(n + 1, vector<int>(2, 0)); // 初始化二维数组f为0
        f[0][1] = INT_MIN;
        for (int i = 0; i < n; i++) {
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (auto &price : prices) {
            int new_f0 = max(f0, f1 + price); // 用new_f0存储 
            f1 = max(f1, f0 - price);
            f0 = new_f0;
        }
        return f0;
