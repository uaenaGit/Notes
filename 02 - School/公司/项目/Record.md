...
箭头不是程序执行顺序，而是表达“两个概念之间有什么关系”。

| 三元组               | 人话解释             |
| ----------------- | ---------------- |
| `LRE --包含--> TC`  | 液体火箭发动机包含推力室     |
| `LRE --包含--> N`   | 液体火箭发动机包含喷管      |
| `TC --执行--> CB`   | 推力室中进行燃烧过程       |
| `CB --消耗--> P`    | 燃烧过程消耗推进剂        |
| `CB --产生--> HG`   | 燃烧过程产生高温燃气       |
| `N --执行--> ACC`   | 喷管中发生对燃气的加速过程    |
| `ACC --加速--> HG`  | 加速过程作用于高温燃气，使其加速 |
| `ACC --产生--> THR` | 加速过程最终产生推力       |

可以把这 8 行想成 8 条结构化事实，而不是一句完整的自然语言描述。
...
最后三条的措辞确实不够自然，尤其是“喷管执行加速过程”。这是 LLM 为了形成图结构，给节点和关系自动命名的结果，不应当把它视为已经人工确认的本体定义。
...
`127.0.0.1` 是回环地址，只能由这台电脑自己访问。因此：
...
```
http://192.168.60.57:5173
```

会被拒绝，因为服务没有在这张局域网网卡上监听。
...
不是把服务绑定到一个具体 IP，而是让 Vite 监听这台机器所有 IPv4 网卡。于是同一个 5173 端口同时能通过：
...
访问。Vite 因此打印了两个 `Network` 地址，分别对应局域网网卡与虚拟网卡。
...
这保证端口被占用时直接报错，不会悄悄换成 5174，避免你以为部署地址仍是 `:5173`。

后端仍监听 `127.0.0.1:8001`，不向局域网暴露。前端调用 API 使用相对路径 `/api/...`，Vite 代理在服务器内部把请求转发到：

```
127.0.0.1:8001
```

所以外部浏览器只需要访问前端 IP 和端口，后端接口仍受本机网络边界保护。