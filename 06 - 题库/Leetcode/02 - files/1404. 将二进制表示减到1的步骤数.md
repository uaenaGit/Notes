...
注意到：从`s.size() - 1` 到 `1`的遍历过程中，不论 `s[i] = 0 or 1`都执行了「除以$2$」操作，可以初始化 `ans = s.size() - 1`。
```cpp
class Solution {
public:
    int numSteps(string s) {
        int ans = s.size() - 1; // 除了最高位 每一位都会执行除以2操作
        int carry = 0;
        // 注意是 i > 0 不是 i >= 0；最高位单独处理
        for (int i = s.size() - 1; i > 0; i--) {
            int sum = carry + (s[i] - '0');
            ans += sum % 2; // 如果 s[i] 变成 1, 需要执行「加上1」操作
            carry = (sum + sum % 2) / 2;
        }
        return ans + carry; // 最后若 carry = 1，还得执行一次除以2操作
    }
};
```
...
>✨ tips
- 由于都要执行 「除以 $2$」操作，初始化`ans = s.size() - 1`，最高位单独处理。
- `carry = (sum + sum % 2) / 2;` 保证了只要 `sum > 0`，`carry`就会变成 $1$。也等价于`carry = sum > 0;`
